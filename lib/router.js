...
var Router = exports = module.exports = function(app, name, options, onMember) {
  ...
  var skips = util.toArray(controller['skip_before_filter'])
  var _this = this
  var args
  ...
  actions.forEach(function(action) {
    args = util.combo(action, controller['before_filter'], skips)
    args.unshift(util.setPath.call(_this, action, onMember))
    args.push(
      util.createCallback(controller[action], namespace, name, action, skips)
    )
